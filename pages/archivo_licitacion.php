...
$query_rsArchivo = sprintf("SELECT * FROM archivos_licitacion WHERE ID_ARCHIVO = %s", GetSQLValueString($colname_rsArchivo, "int"));
...
$file = $row_rsArchivo['lici_archnombre'];
header ("Content-Disposition: attachment; filename=".$file." "); 
header('Content-type: '.$row_rsArchivo['EXTENSION']); // possible but still strange...
//header ("Content-Length: ".filesize($row_rsArchivo['DOCU_ARCHIVO']));//readfile($url); 
echo $row_rsArchivo['ARCHIVO'];
exit;
