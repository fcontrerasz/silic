...
mysql_select_db($database_cn_licitaciones, $cn_licitaciones);
$query_rsListadoArchivos = sprintf("SELECT listar_maestro_archivos_proveedores.*, usuarios.USUPROVEEDOR as NOMEMPRESA FROM listar_maestro_archivos_proveedores inner join usuarios on usuarios.id_usuario = listar_maestro_archivos_proveedores.id_empresa WHERE ID_LICITACION = %s ORDER BY listar_maestro_archivos_proveedores.PROVEEDOR ASC", GetSQLValueString($colname_rsListadoArchivos, "int"));
...
$colname_rs_inserta = $_GET["accion"];
}
//echo $_POST["idinserta"];
//echo "->".limpiar("informaci�n_proy�cto.txt")."<-";
...
$nombre  = normaliza(utf8_decode($_FILES["ARCHIVO"]["name"]));
$idpostulacion = $_POST["idlicitacion"];
$xnombre = normaliza(utf8_decode($_POST["nombre"]));
$xtipo = $_POST["tipo"];

$fp = fopen($archivo, "rb");
$totaltam = filesize($archivo);
/*
echo "INFO NOMBRE -------->".$xnombre."<BR>";
echo "NOMBRE -------->".$_FILES["ARCHIVO"]["name"]."<- new ->".$nombre."<BR>";
...
$irsalto = "panelarchivoslicitaciones.php?ID_LICITACION=".$colname_rsBasesLicitacion."&accion=".$colname_rs_inserta;
header("Location: ".$irsalto);
...
<?php if(in_array($_SESSION['MM_UserGroup'], array("2"))){ ?>
    <table class="flexme2">
		...
		do { ?>
            ...
            <td><?php echo $row_rsListadoArchivos['NOMEMPRESA']; ?></td>
